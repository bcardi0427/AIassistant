...
class AgentSystem:
    ...
    def __init__(self, config_manager: ConfigurationManager, system_prompt: Optional[str] = None, enable_cache_control: bool = False, usage_tracking: str = 'stream_options'):
        ...
        if not api_key:
            ...
            # Check if we're using a Gemini 3 model that requires native API
            model = os.getenv('OPENAI_MODEL', 'gpt-4o')
            self._init_gemini_client_if_needed(api_key, model)
...
Remember: You're helping manage a production Home Assistant system. Safety and clarity are paramount."""
    ...
    def _init_gemini_client_if_needed(self, api_key: str, model: str):
        """Initialize native Gemini client if using Gemini 3+ model.
        
        Gemini 3 and later models require the native Gemini API for proper
        function calling support with thought_signature handling.
        """
        self.gemini_client = None
        ...
        if self._is_gemini_3_model(model):
            try:
                ...
                logger.info(f"Initialized native Gemini client for model: {model}")
            except ImportError as e:
                logger.warning(f"Could not import Gemini client: {e}. Falling back to OpenAI-compatible API.")
            except Exception as e:
                logger.error(f"Failed to initialize Gemini client: {e}. Falling back to OpenAI-compatible API.")

    def _is_gemini_3_model(self, model: str) -> bool:
        """Check if model is Gemini 3 or later (requires native API).
    ...
    async def chat_stream(
        ...
        # Use native Gemini client for Gemini 3+ models
        if hasattr(self, 'gemini_client') and self.gemini_client:
            async for event in self._chat_stream_gemini(user_message, conversation_history):
                yield event
            return
    ...
    async def _chat_stream_gemini(
        ...
        """
        Native Gemini streaming handler for Gemini 3+ models.
        
        Uses the google-genai SDK for proper function calling support.
        """
        import json
        
        logger.info(f"[GEMINI] Streaming user message: {user_message[:100]}...")
        ...
        try:
            ...
            if conversation_history:
                for msg in conversation_history:
                    if isinstance(msg.get("content"), str) or msg.get("tool_calls"):
                        messages.append(msg)
            
            # Add current user message
            messages.append({"role": "user", "content": user_message})
            
            # Define tools in OpenAI format (Gemini client will convert)
            propose_tool = {
                "type": "function",
                "function": {
                    "name": "propose_config_changes",
                    "description": "Propose changes to one or more configuration files for user approval.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "changes": {
                                "type": "array",
                                "description": "List of file changes to propose",
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "file_path": {
            ...
            tools = [
                {
                    ...
                    "function": {
                        "name": "search_config_files",
                        "description": "Search configuration files. Returns matching files with content.",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "search_pattern": {
            ...
            while iteration < max_iterations:
                iteration += 1
                logger.info(f"[GEMINI][ITERATION {iteration}] Calling native Gemini API")
                ...
                async for event in self.gemini_client.generate_content_stream(messages, tools):
                    if event["type"] == "content":
                        ...
                        if content:
                            ...
                            yield {
                                "event": "token",
                                "data": json.dumps({"token": content, "iteration": iteration})
                            }
                        ...
                        raise Exception(event.get("error", "Unknown Gemini error"))
                ...
                # Process tool calls
                logger.info(f"[GEMINI][ITERATION {iteration}] Processing {len(accumulated_tool_calls)} tool call(s)")
                ...
                for tool_call in accumulated_tool_calls:
                    ...
                    logger.info(f"[GEMINI][ITERATION {iteration}] Calling tool: {function_name}")
                    ...
                    if function_name == "search_config_files":
                        ...
                        if "changes" not in function_args:
                            result = {"success": False, "error": "Missing 'changes' parameter"}
                        else:
                            result = await self.tools.propose_config_changes(**function_args)
                    else:
                        result = {"success": False, "error": f"Unknown tool: {function_name}"}
            ...
            # Completion
            if iteration >= max_iterations:
                logger.warning(f"[GEMINI] Hit max iterations ({max_iterations})")
                yield {
                    "event": "error",
                    "data": json.dumps({
            ...
            logger.info(f"[GEMINI] Completed after {iteration} iteration(s)")
